...
{ 
	*(__IO uint16_t *) (Bank1_LCD_R)= regval;//写入要写的寄存器序号	 
}
//写LCD数据
//data:要写入的值
...
{	
	*(__IO uint16_t *) (Bank1_LCD_R) = LCD_Reg;		//写入要写的寄存器序号	 
	*(__IO uint16_t *) (Bank1_LCD_D) = LCD_RegValue;//写入数据	    		 
}	   
//读寄存器
//LCD_Reg:寄存器地址
//返回值:读到的数据
...
{
 	*(__IO uint16_t *) (Bank1_LCD_R)=lcddev.wramcmd;	  
}	 
//LCD写GRAM
//RGB_Code:颜色值
void LCD_WriteRAM(u16 RGB_Code)
{							    
	*(__IO uint16_t *) (Bank1_LCD_D) = RGB_Code;//写十六位GRAM
}
...
u16 LCD_ReadPoint(u16 x,u16 y)
{
	...
	if(*(__IO uint16_t *) (Bank1_LCD_D))r=0;							//dummy Read	   
	opt_delay(2);	  
 	r=*(__IO uint16_t *) (Bank1_LCD_D);  		  						//实际坐标颜色
 	if(lcddev.id==0X9341||lcddev.id==0X5310||lcddev.id==0X5510)		//9341/NT35310/NT35510要分2次读出
 	{
		...
		g<<=8;
	}else if(lcddev.id==0X6804)r=*(__IO uint16_t *) (Bank1_LCD_D);		//6804第二次读取的才是真实值 
...
void LCD_DrawPoint(u16 x,u16 y)
{
	...
	*(__IO uint16_t *) (Bank1_LCD_D)=POINT_COLOR; 
}
//快速画点
//x,y:坐标
//color:颜色
...
{ 										  
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB|RCC_APB2Periph_GPIOD|RCC_APB2Periph_GPIOE|RCC_APB2Periph_AFIO,ENABLE);//??PORTB,D,E,G??AFIO??????
...
/*   	//	//PORTG12?????? A0	
    ...
    writeTiming.FSMC_DataSetupTime = 0x01;		 ////???????4?HCLK	
...
{  
 	...
 	for(i=0;i<height;i++)
	{
		...
		LCD_WriteRAM_Prepare();     //开始写入GRAM
		for(j=0;j<width;j++)*(__IO uint16_t *) (Bank1_LCD_D)=color[i*height+j];//写入数据 
	}	  
}  
//画线
//x1,y1:起点坐标
